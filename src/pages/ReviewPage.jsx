...
import { TrendingUp, TrendingDown, DollarSign, Repeat, Calendar, Calculator, Tag, X, Plus, MessageCircle } from 'lucide-react';
...
const ReviewPage = () => {
  ...
  const activeRecurringIds = new Set(
    monthTransactions.filter(t => (t.recurring || t.originalId) && t.type === 'expense').map(t => t.originalId || t.id)
  );
  const recurringExpenses = transactions.filter(t => activeRecurringIds.has(t.id));

  const historicalBills = transactions.filter(t => 
    t.categoryId === 'cat_bills' && t.type === 'expense' && t.date <= todayIso && t.date >= sixMonthsAgoIso && !t.tags?.includes('חד_פעמי')
  );
  ...
  const customAverages = trackedTags.map(tag => {
    const txs = transactions.filter(t => 
      t.type === 'expense' && t.tags?.includes(tag) && t.date <= todayIso && t.date >= sixMonthsAgoIso && !t.tags?.includes('חד_פעמי')
    );
  ...
  const handleShareWhatsApp = () => {
    ...
    const greeting = balance >= 0 
      ? '✨ סגרנו את החודש בפלוס! איזה אלופים אנחנו! 🎉' 
      : '⚠️ חרגנו קצת החודש, אבל נתאפס בחודש הבא 💪';
      
    const text = `📊 *סיכום ${monthName}* 📊\n\n` +
                 `💵 הכנסות: ${formatCurrency(income)}\n` +
                 `💳 הוצאות: ${formatCurrency(expenses)}\n` +
  ...
  return (
          ...
          <TabsContent value="recurring" className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
            <Card className="clean-card bg-primary/5 border-primary/20">
              <CardContent className="pt-6 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-xs text-muted-foreground mt-2 max-w-xs px-4">משקלל מנויים חודשיים, ממוצע חשבונות, ותגיות אישיות במעקב.</p>
              </CardContent>
            </Card>
            ...
            {recurringExpenses.length === 0 && billsMonthlyAverage === 0 && customAverages.length === 0 ? (
              ...
              <ul className="space-y-3">
                {billsMonthlyAverage > 0 && (
                  <Card className="clean-card border-none shadow-sm relative overflow-hidden cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => openDrillDown('חשבונות (ממוצע)', totalBillsHistorical, historicalBills, 'סך כל החשבונות בחצי השנה האחרונה')}>
                    ...
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center space-x-3 rtl:space-x-reverse">
                        ...
                        <div><p className="font-semibold text-foreground text-sm">חשבונות (ממוצע חודשי)</p><div className="flex items-center text-xs text-muted-foreground mt-0.5 gap-1"><Calculator className="h-3 w-3" /> מחושב אוטומטית (ללא "חד_פעמי")</div></div>
                      </div>
                ...
                {customAverages.map(avg => (
                  <Card key={avg.id} className="clean-card border-none shadow-sm relative overflow-hidden cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => openDrillDown(`תגית: ${avg.title}`, avg.totalHistorical, avg.transactions, `עסקאות עם התגית "${avg.title}" בחצי השנה האחרונה`)}>
                    <div className="absolute top-0 right-0 w-1 h-full bg-primary/40"></div>
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center space-x-3 rtl:space-x-reverse">
                        <div className="p-2.5 rounded-full bg-primary/10"><Tag className="h-5 w-5 text-primary" /></div>
                        <div><p className="font-semibold text-foreground text-sm">{avg.title} (ממוצע)</p><div className="flex items-center text-xs text-muted-foreground mt-0.5 gap-1"><Calculator className="h-3 w-3" /> ממוצע נע ל-6 חודשים</div></div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="font-bold text-red-600 dark:text-red-400" dir="ltr">{formatCurrency(avg.average)}</div>
                        <button onClick={(e) => { e.stopPropagation(); removeTrackedTag(avg.id); }} className="p-1 hover:bg-destructive/10 text-muted-foreground hover:text-destructive rounded-full transition-colors" title="הסר ממעקב"><X className="h-4 w-4" /></button>
                      </div>
                    </CardContent>
                  </Card>
                ))}
                    ...
                    <Card key={transaction.id} className="clean-card border-none shadow-sm">
                      <CardContent className="p-4 flex items-center justify-between">
                        <div className="flex items-center space-x-3 rtl:space-x-reverse">
                          ...
                          <div><p className="font-semibold text-foreground text-sm">{transaction.description || categoryInfo?.name_he || 'הוצאה קבועה'}</p><div className="flex items-center text-xs text-muted-foreground mt-0.5 gap-1"><Calendar className="h-3 w-3" /> {frequencyText[transaction.recurrenceFrequency] || transaction.recurrenceFrequency}</div></div>
                        </div>
            ...
            <button onClick={() => setIsTagDialogOpen(true)} className="w-full mt-4 flex items-center justify-center p-3 border-2 border-dashed border-primary/30 rounded-lg text-primary hover:bg-primary/5 transition-colors font-medium text-sm">
              <Plus className="h-4 w-4 mr-2" /> הוסף תגית למעקב ממוצע חודשי
            </button>
          </TabsContent>
        </Tabs>
      </div>
    ...
    <Dialog open={isTagDialogOpen} onOpenChange={setIsTagDialogOpen}>
      <DialogContent className="max-w-[90vw] sm:max-w-md" dir="rtl">
        <DialogHeader><DialogTitle className="text-right">איזו תגית לעקוב?</DialogTitle></DialogHeader>
        <p className="text-sm text-muted-foreground">בחרי תגית קיימת. המערכת תחשב אוטומטית את הממוצע החודשי שלה ותציג אותו בהוצאות הקבועות.</p>
        <div className="flex flex-wrap gap-2 mt-4">
          {availableTagsToAdd.length === 0 ? <p className="text-sm text-muted-foreground w-full text-center py-4">אין תגיות זמינות למעקב כרגע.</p> : availableTagsToAdd.map(tag => (
            <button key={tag} onClick={() => addTrackedTag(tag)} className="px-3 py-1.5 bg-secondary text-secondary-foreground hover:bg-primary hover:text-primary-foreground rounded-full text-sm font-medium transition-colors">{tag}</button>
          ))}
        </div>
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={drillDownData.isOpen} onOpenChange={(isOpen) => setDrillDownData(prev => ({ ...prev, isOpen }))}>
      <DialogContent className="max-w-[95vw] sm:max-w-md max-h-[85vh] overflow-y-auto" dir="rtl">
        <DialogHeader><DialogTitle className="text-right text-xl">{drillDownData.title}</DialogTitle></DialogHeader>
        ...
        <div className="space-y-2 mt-4">
          ...
          {drillDownData.transactions.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">לא נמצאו עסקאות.</p> : drillDownData.transactions.map(t => {
               ...
               return (
                <div key={t.id} className="flex justify-between items-center p-3 bg-secondary/30 rounded-md border border-secondary/50">
                  <div><p className="font-medium text-sm text-foreground">{t.description || catInfo?.name_he || 'ללא תיאור'}</p><p className="text-xs text-muted-foreground">{formatDateHe(t.date)}</p></div>
